...
if [ -n "$CLAUDE_ENV_FILE" ]; then
    ...
    if [ -f "accbmad/config.yaml" ]; then
        echo 'export BMAD_PROJECT=true' >> "$CLAUDE_ENV_FILE"

        # Extract project name from config
        PROJECT_NAME=$(grep "project_name:" accbmad/config.yaml 2>/dev/null | cut -d: -f2 | tr -d ' "')
        if [ -n "$PROJECT_NAME" ]; then
            echo "export BMAD_PROJECT_NAME=\"$PROJECT_NAME\"" >> "$CLAUDE_ENV_FILE"
        fi

        # Extract project level
        PROJECT_LEVEL=$(grep "project_level:" accbmad/config.yaml 2>/dev/null | cut -d: -f2 | tr -d ' ')
        if [ -n "$PROJECT_LEVEL" ]; then
            echo "export BMAD_PROJECT_LEVEL=$PROJECT_LEVEL" >> "$CLAUDE_ENV_FILE"
        fi

        # Set output folder
        OUTPUT_FOLDER=$(grep "output_folder:" accbmad/config.yaml 2>/dev/null | cut -d: -f2 | tr -d ' "')
